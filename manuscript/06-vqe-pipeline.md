...
// Single excitation with parameter θ
ry(theta) q[0];
cx q[0], q[1];
```
...
Molecule + geometry + basis set
    ↓
Classical chemistry code (PySCF, Psi4)
    → molecular integrals h_ij, h_ijkl
    ↓
Fermion-to-qubit encoding (Jordan-Wigner, Bravyi-Kitaev)
    → Pauli Hamiltonian H = Σ g_k P_k
    ↓
Choose ansatz (UCCSD, hardware-efficient, ...)
    ↓
┌─── Quantum-classical loop ───────────────────┐
│  Prepare |ψ(θ)⟩ on quantum computer         │
│  Measure each P_k  →  compute ⟨H⟩           │
│  Classical optimiser updates θ               │
│  Repeat until ⟨H⟩ converges                 │
└──────────────────────────────────────────────┘
    ↓
Ground-state energy E₀ ≈ min_θ ⟨H⟩
```
